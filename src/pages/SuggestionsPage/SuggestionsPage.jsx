...
import SuggestionCard from "../../components/cardSuggestion/cardSuggestion";
...
export default function SuggestedFriends() {
  ...
  const { data, isLoading, hasNextPage, fetchNextPage, isFetchingNextPage } =
    useSuggestions(searchValue,10);
  const suggestions =
    data?.pages.flatMap((page) => page.data.suggestions) || [];

  return (
    <>
      <section className="rounded-2xl border p-4 shadow-sm max-w-4xl mx-auto">
        {/* Header */}
        ...
        <label className="relative mb-4 block">
          ...
          <input
            ...
            className="w-full rounded-xl border py-2.5 pl-10 pr-3 text-sm text-slate-700 dark:text-white outline-none focus:border-[#1877f2] dark:focus:bg-slate-900 focus:bg-slate-50 transition-all"
          />
        </label>
